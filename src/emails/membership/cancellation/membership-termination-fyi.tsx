...
const BODY_COPY: Record<TerminationContext, (subjectName: string) => string> = {
  resigned: (name) =>
    `${name} has cancelled their START Berlin e.V. membership. Their account has been closed and all associated data has been removed.`,
  removed_by_board: (name) =>
    `${name}'s membership in START Berlin e.V. has been terminated by board resolution. Their account has been closed and all associated data has been removed.`,
  alumni: (name) =>
    `${name} has cancelled their START Berlin e.V. membership and transitioned to alumni status. Their active account has been closed; alumni access (if any) remains in place.`,
};
