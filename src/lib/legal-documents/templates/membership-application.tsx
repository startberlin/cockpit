...
function DeclarationItem({ text }: { text: string }) {
  return (
    <View style={{
      flexDirection: "row", gap: "3mm",
      alignItems: "flex-start",
      paddingTop: "3mm",
      borderTopWidth: 1, borderTopColor: C.ruleSoft,
    }}>
      <View style={{
        width: "5mm", height: "5mm", flexShrink: 0,
        borderWidth: 1.5, borderColor: C.ink,
        alignItems: "center", justifyContent: "center",
      }}>
        <Text style={{ fontFamily: "Avenir", fontWeight: 900, fontSize: 9, color: C.brand, lineHeight: 1 }}>×</Text>
      </View>
      <Text style={{
        fontFamily: "Avenir", fontWeight: 400,
        flex: 1, fontSize: 8.5, lineHeight: 1.4, color: C.ink,
      }}>{text}</Text>
    </View>
  );
}
...
export function renderMembershipApplicationTemplate(
  ...
  const declarations = DECLARATION_ORDER
    .filter((key) => data.declarations[key])
      ...
      <Page size="A4" style={PAGE_STYLE}>
        <View style={WRAPPER}>
          ...
          <View style={CONTENT}>
            ...
            <Lead>
              I hereby apply for admission as an ordinary member of START
              Berlin e.V. The information below is complete and truthful.
            </Lead>
            ...
            <View style={{ marginTop: "11mm" }}>
              ...
              <View style={{ flexDirection: "row", gap: "14mm" }}>
                <View style={{ flex: 1 }}>
                  ...
                  <FieldRow label="Postal code / City" value={`${data.address.zip} ${data.address.city}`} />
                </View>
                <View style={{ flex: 1 }}>
                  <FieldRow label="State"   value={data.address.state || undefined} />
            ...
            <View style={{ marginTop: "11mm" }}>
              ...
              <View style={{ flexDirection: "row", gap: "10mm" }}>
                <View style={{ flex: 1, gap: "4mm" }}>
                  {leftDecls.map((decl, i) => <DeclarationItem key={i} text={decl} />)}
                </View>
                <View style={{ flex: 1, gap: "4mm" }}>
                  {rightDecls.map((decl, i) => <DeclarationItem key={i} text={decl} />)}
                </View>
              </View>
            </View>
