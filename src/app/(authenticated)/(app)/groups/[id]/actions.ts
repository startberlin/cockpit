...
export async function addUserToGroupAction(userId: string, groupId: string, role: "admin" | "member" = "member"): Promise<void> {
  await addUserToGroupRaw(userId, groupId, role);
  revalidatePath(`/groups/${groupId}`);
}

export async function removeUserFromGroupAction(userId: string, groupId: string): Promise<void> {
  await removeUserFromGroupRaw(userId, groupId);
  revalidatePath(`/groups/${groupId}`);
}

export async function updateUserGroupRoleAction(userId: string, groupId: string, role: "admin" | "member"): Promise<void> {
