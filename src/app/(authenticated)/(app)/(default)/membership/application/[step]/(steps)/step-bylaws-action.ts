...
export const saveBylawsDeclarationsAction = actionClient
  ...
  .action(async ({ ctx, parsedInput }) => {
    ...
    await db.transaction(async (tx) => {
      ...
      const updated = await tx
        ...
        .set({
          declarations: sql`COALESCE(${membershipApplication.declarations}, '{}') || ${JSON.stringify({ supportsPurpose: true, acceptsBylaws: true } satisfies Partial<MembershipApplicationDeclarations>)}::jsonb`,
        })
        .where(
          and(
            eq(
